...
from .._styling import PALETTE, THEMES, hex_to_rgba, apply_theme
...
METRIC_CONFIG = {
    ...
    'trend': {'color': '#4A90A4', 'threshold': 0.5, 'label': 'Strong Trend', 'xlim': (0, 1)},
    'seasonal_strength': {'color': '#4A90A4', 'threshold': 0.5, 'label': 'Strong Seasonality', 'xlim': (0, 1)},
    'x_acf1': {'color': '#4A90A4', 'threshold': 0.6, 'label': 'Strong Persistence', 'xlim': (-0.5, 1)},
    'structure_score': {'color': '#4A90A4', 'threshold': 0.5, 'label': 'High Structure', 'xlim': (0, 1)},
    # Chaos metrics (orange)
    'entropy': {'color': '#D4775D', 'threshold': 0.8, 'label': 'Chaotic', 'xlim': (0, 1)},
    'adi': {'color': '#D4775D', 'threshold': 1.32, 'label': 'Intermittent', 'clip_quantile': 0.95},
    'cv2': {'color': '#D4775D', 'threshold': 0.49, 'label': 'High Variability', 'clip_quantile': 0.95},
    'lumpiness': {'color': '#D4775D', 'threshold': 1.0, 'label': 'Lumpy', 'clip_quantile': 0.95},
    'chaos_score': {'color': '#D4775D', 'threshold': 0.5, 'label': 'High Chaos', 'xlim': (0, 1)},
}
...
def _plot_metric_distribution(
    ...
    if use_metric_defaults and color_col is None and group_col is None:
        ...
        if clip_quantile is None:
            clip_quantile = {c: METRIC_CONFIG[c].get('clip_quantile') for c in columns if c in METRIC_CONFIG and 'clip_quantile' in METRIC_CONFIG[c]}
    ...
    # Handle group_col with facet_by/dropdown_by control
    if group_col is not None and (facet_by is not None or dropdown_by is not None):
    ...
    if mode == "facet":
        for i, col in enumerate(columns):
            ...
            if show_median:
                ...
                fig.add_vline(
                    x=median_val, line_dash="dash", line_color=color, line_width=1.5,
                    row=(i // wrap) + 1, col=(i % wrap) + 1,
            ...
            if show_mean:
                mean_val = data.mean()
                fig.add_vline(
                    x=mean_val, line_dash="dot", line_color="gray", line_width=1.5,
                    row=(i // wrap) + 1, col=(i % wrap) + 1,
                )
            ...
            if col in thresholds:
                ...
                if show_threshold_pct:
                    ...
                    annotation_text = f"{label} {pct_above:.1f}%" if label else f"{pct_above:.1f}%"

                fig.add_vline(
                    x=thresh, line_dash="solid", line_color=threshold_color, line_width=2,
                    row=(i // wrap) + 1, col=(i % wrap) + 1,
    ...
    # For overlay mode, add lines to single plot
        ...
        for i, col in enumerate(columns):
            ...
            if col in thresholds:
                thresh = thresholds[col]
                fig.add_vline(
                    x=thresh, line_dash="solid", line_color=color, line_width=2,
...
def _plot_metric_distribution_facet_dropdown(
    ...
    # Create subplots
    subplot_titles = [get_facet_title(item) for item in facet_items] if facet_items[0] is not None else None
    fig = make_subplots(
        rows=rows,
        cols=cols,
    ...
    # Add threshold lines (for faceted columns)
    if facet_by == "columns":
        for facet_idx, col in enumerate(columns):
            if col in thresholds:
                thresh = thresholds[col]
                row = (facet_idx // cols) + 1
...
def _plot_metric_distribution_grouped(
    ...
    # Add threshold lines for facet mode
    if mode == "facet":
        for i, col in enumerate(columns):
            if col in thresholds:
                thresh = thresholds[col]
...
def _plot_timeseries_distribution(
    ...
    for i, uid in enumerate(ids_list):
        ...
        if kind == "histogram":
            traces_by_id[uid] = [go.Histogram(
                x=sub[plot_col], name=str(uid), opacity=0.7,
                nbinsx=bins, marker=dict(color=color),
            )]
        elif kind == "density":
            ...
            traces_by_id[uid] = [go.Box(
                y=sub[plot_col], name=str(uid),
                marker=dict(color=color), boxpoints="outliers",
            )]
        elif kind == "violin":
            traces_by_id[uid] = [go.Violin(
                y=sub[plot_col], name=str(uid),
                line=dict(color=color), box_visible=True, meanline_visible=True,
            )]
        else:
